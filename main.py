...
def sendnotif(tradejson):
    ...
    DiscordID = data['Discord ID'] #I call these from within the subroutine incase you change them midway through the code.
    RequestEmoji = data['RequestEmoji']
    OfferEmoji = data['OfferEmoji']
    WinEmoji = data['WinEmoji']
    LoseEmoji = data['LoseEmoji']
    TradeValues = [0,0]
        ...
        [
            {
                ...
                "fields": [
                    ...
                    {
                        "name": f"Requesting {RequestEmoji}", #This might be a bit confusing, sorry! Essentially, I add a :warning: if the item is projected, then I post the item name and a link to that item - "\n".join() is just an easier way of sending them all in separate lines
                    ...
                    },
                    {
                        ...
                        "value": f"{GainOrLoss[0]} Value {GainOrLoss[1]}: {abs(TradeValues[1]-TradeValues[0]):,d}\n{GainOrLoss[0]} Percentage {GainOrLoss[1]}: {'+' if (TradeValues[1]-TradeValues[0])>=0 else '-'}{round(abs(((TradeValues[1]-TradeValues[0])/TradeValues[0])*100),2)}%\n{ItemGainOrLoss[0]} Item {ItemGainOrLoss[1]}: {abs(len(tradejson['offers'][1]['userAssets'])-len(tradejson['offers'][0]['userAssets'])):,d}"
                    }
                ], 
                "thumbnail": {
    ...
    x = requests.post(url,json=data) #Posts the webhook
    print(x.status_code)
